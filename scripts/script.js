    ...
    $(document).bind('keydown', function (e) {
        ...
        if (e.keyCode == 13) {
            if (formState === 0) {
                ...
                jquery_str = '.' + formStages[formState] + ' > .stage__container--step-four > .stage__buttons > .button--submit'
                $(jquery_str).trigger('click');
            } else {
                jquery_str = '.' + formStages[formState] + ' > .stage__buttons > .button--next'
                $(jquery_str).trigger('click');
            } 
        }
    });
    ...
    if ('week' in mealsData) {
        ...
        for(i = 0; i <= 2; i++){
            ...
            const day_calories = mealsData.nutrients.calories;
                ...
                <div class="recipes__recipe-card">
                    <div class="recipe-card__image">
                        <img src="https://spoonacular.com/recipeImages/${id}-636x393.jpg" alt="A shot of the finished result of the ${title} recipe">
                    </div>
                    <div class="recipe-card__description">
                        <h3 class="main-header main-header--recipe">${title}</h3>
                        <a href="${link}" class="main-paragraph main-paragraph--external-link" target="_blank" rel="external" alt="This link will take you to an external website that contains the ${title} recipe">Check out the full recipe now</a>
                    </div>
                </div> 
            `);
        }
